...
static cv::Point2f interp(const std::vector<cv::Point2f> &curve, float x)
{
    const int i(x);
    const float b = x - float(i);
    const float a = (1.0f - b);
    return (curve[i+0] * a + curve[i+1] * b);
}

void CurveNormalizer::operator()(const cv::Mat &crop, const std::vector<cv::Point2f> &curve, const cv::Size &size, NormalizedCurve &code) const
{
    // Defines the resolution:
    float r = float(curve.size()) / float(size.width);

    cv::Size paddedSize = size;

    code.getImage().create(size, crop.type());
    cv::Mat1f mapX(size), mapY(size);

    int h0 = size.height / 2, h1 = size.height - h0;
    for(int x = 1; x < size.width-1; x++)
    {
        const cv::Point2f p = interp(curve, float(x) * r);
        const cv::Point2f p0 = interp(curve, float(x-1) * r);
        const cv::Point2f p1 = interp(curve, float(x+1) * r);
        const cv::Point2f d(p0 - p1), n(d.y, -d.x), v = cv::normalize(cv::Vec2f(n));
        for(int y = -h0; y < h1; y++)
        {
            cv::Point2f q = p + (v * float(y) * r);
            mapX(y+h0, x) = q.x;
            mapY(y+h0, x) = q.y;
        }
    }

    cv::Mat output(size, crop.type());
    cv::remap(crop, output, mapX, mapY, cv::INTER_CUBIC);

    cv::Mat hsi, channels[3];
    cv::cvtColor(output, hsi, cv::COLOR_BGR2HSV_FULL);
    cv::split(hsi, channels);
    cv::equalizeHist(channels[2], channels[2]);
    cv::merge(channels, 3, hsi);
    cv::cvtColor(hsi, output, cv::COLOR_HSV2BGR_FULL);

    code.getImage() = output;
