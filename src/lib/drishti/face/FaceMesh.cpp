...
std::array<cv::Mat1f, 2> FaceMesh::transform(const Triangles &a, const Triangles &b, const cv::Size &size)
{
    ...
    {
        const auto &tA = a[i];
        std::vector<std::vector<cv::Point>> pointsA
        {{
            cv::Point(tA[0],tA[1]),
            cv::Point(tA[2],tA[3]),
            cv::Point(tA[4],tA[5])
        ...
        const auto &tB = b[i];
        std::vector<std::vector<cv::Point>> pointsB
        {{
            cv::Point(tB[0],tB[1]),
            cv::Point(tB[2],tB[3]),
            cv::Point(tB[4],tB[5])
...
FaceMesh::Triangles FaceMesh::delaunay(const Landmarks &landmarks, const cv::Size &size, bool doHalf)
{
    ...
    else
    {
        ...
        for(int i = 0; i < trianglesL.size(); i++)
        {
            ...
            int k1 = 0, k2 = 0, k3 = 0;
            for(k1 = 0; k1 < kMirrorMap.size(); k1++) { if(landmarks[kMirrorMap[k1][0]] == p1) break; }
            for(k2 = 0; k2 < kMirrorMap.size(); k2++) { if(landmarks[kMirrorMap[k2][0]] == p2) break; }
            for(k3 = 0; k3 < kMirrorMap.size(); k3++) { if(landmarks[kMirrorMap[k3][0]] == p3) break; }
