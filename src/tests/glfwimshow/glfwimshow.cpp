...
std::pair<cv::Mat, std::string> create(const cv::Size &size, int type, const std::string &name)
{
    ...
    switch (type)
    {
        case CV_8U:  maxVal = std::numeric_limits<std::uint8_t>::max(); break;
        case CV_8S:  maxVal = std::numeric_limits<std::int8_t>::max(); break;
        case CV_16U: maxVal = std::numeric_limits<std::uint16_t>::max(); break;
        case CV_16S: maxVal = std::numeric_limits<std::int16_t>::max(); break;
        case CV_32S: maxVal = std::numeric_limits<std::int32_t>::max(); break;
        case CV_32F: maxVal = 1.0f; break;
