...
#ifndef WEBSOCKET_ASYNC_SERVER_HPP
#define WEBSOCKET_ASYNC_SERVER_HPP
...
public:
    ...
    using streambuf_handler = std::function<int(beast::streambuf&)>;

    void add(streambuf_handler handler)
    {
        handler_ = handler;
    }

private:
    streambuf_handler handler_;
    ...
    struct identity
    {
        ...
        {
            req.fields.replace("User-Agent", "async_client");
        }

        template <class Body, class Fields>
        void operator()(beast::http::message<false, Body, Fields>& resp) const
        {
            resp.fields.replace("Server", "async_server");
        }
    };
    ...
    public:
        ...
        void set_options(beast::websocket::stream<NextLayer>& ws)
        {
...
public:
    ...
    */
    async_server(std::ostream* log, std::size_t threads)
        : log_(log)
        , sock_(ios_)
        , acceptor_(ios_)
        , work_(ios_)
    {
        opts_.set_option(beast::websocket::decorate(identity{}));
    ...
    */
    void open(endpoint_type const& ep, error_code& ec)
    {
        ...
        acceptor_.async_accept(sock_, ep_, std::bind(&async_server::on_accept, this, beast::asio::placeholders::error));
    }
...
private:
    class peer
    {
        struct data
        {
            ...
            data(async_server& server_, endpoint_type const& ep_, socket_type&& sock_)
                : server(server_)
                , ep(ep_)
                , ws(std::move(sock_))
                , strand(ws.get_io_service())
                , id([] { static std::atomic<std::size_t> n{0}; return ++n; }())
            {
            }
        };
    ...
    public:
        ...
        template <class... Args>
        explicit peer(async_server& server, endpoint_type const& ep, socket_type&& sock, Args&&... args)
            : d_(std::make_shared<data>(server, ep, std::forward<socket_type>(sock), std::forward<Args>(args)...))
        {
            auto& d = *d_;
        ...
        void operator()(error_code ec)
        {
            ...
            switch (d.state)
            {
                ...
                case 2:
                    ...
                    if (d.op == beast::websocket::opcode::binary)
                    {
                        if (d.server.handler_)
                        {
                            d.server.handler_(d.db);
                        }
                    }

                    // write message
                    d.state = 1;
                    d.ws.set_option(beast::websocket::message_type(beast::websocket::opcode::text));
                    d.ws.write(boost::asio::buffer(std::string("OK!")));
                    return;
            }
        }
    ...
    private:
        void fail(std::string what, error_code ec)
        {
            ...
            if (d.server.log_)
            {
                if (ec != beast::websocket::error::closed)
                {
                    d.server.fail("[#" + std::to_string(d.id) + " " + boost::lexical_cast<std::string>(d.ep) + "] " + what, ec);
                }
            }
        }
    };
